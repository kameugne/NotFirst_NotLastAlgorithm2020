 ...
 *
 **/
public class HorizontallyEdgeFinderRevisited {
    Integer C;
    Task[] tasks;
    private Profile tl;
    private int[] Prec;
    ...
    public int[] Filter(){
        ...
        if(RevisitedEdgeFinder_Detection()){
            result = RevisitedEdgeFinder_Prunning();
        }
        return result;
    }


    public HorizontallyEdgeFinderRevisited(Task[] tasks, int C)
    {
        this.C = C;
        this.tasks = tasks;
        this.tl = new Profile();
        ...
        for(int i = tasks_indices_lct.length -1; i > 0; i--)
        {
            ...
            while(max <= tasks.length)
            {
                if(max == tasks.length || tasks[tasks_indices_h[max]].height() > tasks[tasks_indices_h[min]].height()){
                    int h = -1; int k = min;
                    while(k < max && h == -1){
                        if(tasks[tasks_indices_h[k]].inLambda && tasks[tasks_indices_h[k]].earliestCompletionTime() < tasks[tasks_indices_h[k]].latestCompletionTime())
                    ...
                    if(h != -1){
                        ...
                        if(ScheduleTasks(tasks[tasks_indices_lct[i-1]].latestCompletionTime(), tasks[tasks_indices_h[min]].height()) > tasks[tasks_indices_lct[i-1]].latestCompletionTime())
                            return false;
                        DetectePrecedences(i-1, k-1, max-1);
                    }
                    min = max;
                }
                max++;
