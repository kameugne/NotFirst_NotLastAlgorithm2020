 ...
 *
**/
public class HorizontallyEdgeFinder {
    private Integer C;
    private Integer n;
    private Task[] tasks;
    private Profile tl;
    ...
    public HorizontallyEdgeFinder(Task[] tasks, int C)
    {
        this.C = C;
        this.n = tasks.length;
        this.tasks = tasks;
    ...
    private boolean EdgeFinder_Detection()
    {
        ...
        for(int i = n-1; i > 0; i--)
        {
            if (tasks[tasks_indices_lct[i]].inLambda &&
                    tasks[tasks_indices_lct[i]].earliestCompletionTime() < tasks[tasks_indices_lct[i]].latestCompletionTime()) {
                ...
                while (j < n && !isDetected) {
                    if (tasks[tasks_indices_lct[j]].latestCompletionTime() < tasks[tasks_indices_lct[i]].latestCompletionTime()) {
                        ...
                        if (t.overflow > t.energy - estEnergyOverflow - Energy) {
                            if (tasks[tasks_indices_lct[j]].latestCompletionTime() <= tasks[tasks_indices_lct[i]].earliestStartingTime())
                                return false;
                            else {
